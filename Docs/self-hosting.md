# Self-Hosting Domaeng

This guide is for developers who clone the public GitHub repository and want to run Domaeng on infrastructure they control.
...
## What Domaeng Self-Hosting Means
...
- your browser is a remote control
...
- a browser on this Mac or another device
...
1. Open the relay-served web app
...
### If another device cannot reach the default hostname

Pass a hostname or IP address that the device can actually reach:

```sh
./run-local-domaeng.sh --hostname 192.168.1.10
```
...
Copy the generated `https://<random>.trycloudflare.com` URL, then start Domaeng in another terminal:
...
This is also the best base for a Tailscale setup: the relay can live on a Mac, a mini server, or a VPS you control, as long as your browser can reach it reliably.
...
- a shared-domain subpath, for example `wss://api.example.com/domaeng/relay`
...
```sh
DOMAENG_RELAY="wss://relay.example.com/relay" domaeng up
```
...
cd phodex-bridge
npm install
DOMAENG_RELAY="wss://relay.example.com/relay" npm start
```

The bridge will print a QR code the first time you trust that Mac, or later if you intentionally reset trust.

That QR carries the relay URL and session information, so the web app does not need a hardcoded relay endpoint in the public source build.

After the first successful scan:

- the web app stores the Mac as a trusted device
...
- you do not need push to use Domaeng
...
- a bridge-side `DOMAENG_PUSH_SERVICE_URL`
...
- only enable `DOMAENG_TRUST_PROXY=true` when the proxy is trusted and sanitizes forwarded IP headers
...
- any private package build defaults
...
### The bridge starts but another device cannot connect
...
If local LAN pairing still fails on another device even though the relay health check works, prefer a Tailscale-reachable relay instead of continuing to rely on plain `ws://` over the same Wi-Fi.
...
- prefer a relay path reachable from your browser over Tailscale or another stable private network
- point the bridge at your relay with `DOMAENG_RELAY`
- scan the QR or enter the pairing code from the web app once to trust the Mac
